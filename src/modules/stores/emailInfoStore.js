...
export const EmailInfoStore = observable({
  ...
  setEmailBody({
    ...
    if (!actionTime || !developer || !branch || !emailMessage || !projectName) return logError(
      'Setting email header failed:',
      'No actionTime | developer | branch | emailMessage | projectName'
    )
    let incommingEmailMsg = emailMessage

    let isAutomatic = false

    if (typeof incommingEmailMsg === 'string') {
      isAutomatic = incommingEmailMsg.includes('AUTOGENERATED')
      incommingEmailMsg = {
        head: incommingEmailMsg,
        body: ''
      }
    } else isAutomatic = incommingEmailMsg.head.includes('Automatic commit')
    ...
    const emailHead = isAutomatic ? '' : `${incommingEmailMsg.head}\n`

    const emailBody = isAutomatic
      ? `❍ ${developerName} worked on the task \n`
      : incommingEmailMsg.body.split('-').join('').split('☐☐').join('☐ ... ☐')

    const fullDescription = `${emailHead}${emailBody}`
    ...
    <code><pre>${incommingEmailMsg.head}<br /></pre></code>
    <code><pre>${incommingEmailMsg.body}</pre></code>
    <br />
    <hr />`
