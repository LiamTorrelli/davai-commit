...
export const EmailInfoStore = observable({
  ...
  setEmailBody({
    ...
    let incommingCommitMsg = commitMessage

    let isAutomatic = false

    if (typeof incommingCommitMsg === 'string') {
      isAutomatic = incommingCommitMsg.includes('AUTOGENERATED')
      incommingCommitMsg = {
        head: incommingCommitMsg,
        body: ''
      }
    } else isAutomatic = incommingCommitMsg.head.includes('Automatic commit')
    ...
    const commitHead = isAutomatic ? '' : `${incommingCommitMsg.head}\n`

    const commitBody = isAutomatic
      ? `❍ ${developerName} worked on the task \n`
      : incommingCommitMsg.body.split('-').join('').split('☐☐').join('☐ ... ☐')
    ...
    <code><pre>${incommingCommitMsg.head}<br /></pre></code>
    <code><pre>${incommingCommitMsg.body}</pre></code>
    <br />
    <hr />`
